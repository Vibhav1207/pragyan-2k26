...
export const RedesignedEventAtAGlance: React.FC<RedesignedEventAtAGlanceProps> = ({ className = '' }) => {
  return (
    <div className={`w-full rounded-3xl bg-[#F3EDE0] border border-[#D2CAB6] p-5 sm:p-6 xl:p-8 shadow-xl relative overflow-hidden text-left ${className}`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 relative z-10 border-b border-[#D2CAB6]/80 pb-4 sm:pb-5">
        <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-3.5 xl:gap-4 my-5 sm:my-6 relative z-10">

        <div className="bg-[#F9F4EA] border border-[#D2CAB6] rounded-2xl p-4 sm:p-3.5 md:p-4 xl:p-5 flex flex-col justify-between relative overflow-hidden hover:border-[#A77A1C] transition-all duration-300 group shadow-sm">
          <div>
            <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-[#E9E1D2] border border-[#A77A1C]/30 flex items-center justify-center mb-3 sm:mb-4 group-hover:bg-[#162E28] group-hover:text-[#F9F4EA] transition-colors duration-300">
              <Users className="w-5 h-5 text-[#A77A1C] group-hover:text-[#E5BE61] transition-colors" />
            </div>

            <div className="font-mono text-[10px] font-bold text-[#7B8379] tracking-widest uppercase mb-1 truncate">
              TEAM COMPOSITION
            </div>

            <div className="flex flex-wrap items-baseline gap-x-1.5 gap-y-0 mb-1">
              <span className="font-serif font-black text-2xl xl:text-3xl text-[#162E28] tracking-tight leading-none">
                <AnimatedCounter value={4} />
              </span>
              <span className="font-serif font-bold text-sm sm:text-xs md:text-sm xl:text-base text-[#162E28] uppercase tracking-tight leading-none">
                MEMBERS
              </span>
            </div>

            <div className="font-sans text-[10px] sm:text-[9.5px] md:text-[10.5px] xl:text-[11px] font-semibold text-[#A77A1C] uppercase tracking-wider">
              PER DELEGATE TEAM
            </div>
          </div>

          <div className="w-8 h-0.5 bg-[#A77A1C] rounded-full mt-3 sm:mt-4" />
        </div>

        <div className="bg-[#162E28] border border-[#A77A1C] rounded-2xl p-4 sm:p-3.5 md:p-4 xl:p-5 flex flex-col justify-between relative overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 group">
          <div>
            <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-[#2B3E35] border border-[#E5BE61]/40 flex items-center justify-center mb-3 sm:mb-4 group-hover:scale-105 transition-transform duration-300">
            ...
            <div className="font-mono text-[10px] font-bold text-[#E5BE61]/80 tracking-widest uppercase mb-1 truncate">
              FIRST ROUND FEE
            </div>

            <div className="flex flex-wrap items-baseline gap-x-0.5 mb-1">
              <span className="font-serif font-black text-2xl xl:text-3xl text-[#E5BE61] tracking-tight leading-none">
                <AnimatedCounter value={500} prefix="₹" />
              </span>
            </div>

            <div className="font-sans text-[10px] sm:text-[9.5px] md:text-[10.5px] xl:text-[11px] font-semibold text-[#F9F4EA]/90 uppercase tracking-wider">
              REGISTRATION PER TEAM
            </div>
          </div>

          <div className="w-8 h-0.5 bg-[#E5BE61] rounded-full mt-3 sm:mt-4" />
        </div>
