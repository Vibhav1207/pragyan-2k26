  ...
  return (
    <section id="timeline" className="w-full py-20 lg:py-28 px-4 sm:px-8 lg:px-12 xl:px-16 bg-[#F0F4FA] text-[#0B192C] text-left border-b border-slate-200">
      <div className="max-w-[1500px] mx-auto w-full space-y-12">
        ...
        {/* Timeline Grid (Phase 1 & Phase 2 in Matching Color Palette) */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {FLYER_TIMELINE.map((phaseData, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-white text-[#0B192C] border border-slate-200 rounded-3xl p-8 space-y-6 flex flex-col justify-between shadow-lg hover:shadow-xl hover:border-[#1D4ED8]/30 transition-all duration-300"
              >
                <div className="space-y-6">
                  
                  {/* Phase Badge Header */}
                  <div className="flex flex-wrap items-center justify-between pb-4 border-b border-slate-200">
                    <span className="bg-[#1D4ED8] text-white font-mono text-xs font-extrabold px-3.5 py-1.5 rounded-full uppercase tracking-wider">
                      {phaseData.badge}
                    </span>
                  ...
                  <div className="space-y-4">
                    {phaseData.events.map((evt, eIdx) => (
                      <div
                        key={eIdx}
                        className="p-6 rounded-2xl bg-[#F0F4FA] border border-slate-200 space-y-3"
                      >
